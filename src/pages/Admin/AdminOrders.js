...
const AdminOrders = () => {
  ...
  const getOrders = async () => {
    try {
      const { data } = await axios.get("https://ssdtest.onrender.com/api/v1/auth/all-orders");
      setOrders(data);
    } catch (error) {
      console.log(error);
    }
  ...
  const handleChange = async (orderId, value) => {
    try {
      const { data } = await axios.put(`https://ssdtest.onrender.com/api/v1/auth/order-status/${orderId}`, {
        status: value,
      });
      getOrders();
    } catch (error) {
